...
//Add Brand API
router.post('/addBrand', verify, async (req, res) => {
    try {
        //Validate the Brand inputed data
        ...
        if(!brand) {
            ...
            const saveBrand = await brand.save()
                .then(() => {
                    res.send(new itemAdded().getMessage('Brand added Successfully.'));
                    var employeeId = 
                        jwt.decode(
                    ...
                    Logger(employeeId, 'Added a Brand.', ip);
                })
                .catch((err) => {
            ...
            //Brand found
            res.send(new addItemError().getErrorMessage('Brand with same name already exist.'));
            return;
        }
    } catch (error) {
